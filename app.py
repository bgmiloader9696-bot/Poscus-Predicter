...
def key_verify():
    data = request.json or {}
    k = (data.get("key") or "").strip().upper()
    device = (data.get("device") or "").strip().upper()
    keys = load_keys()

    # Case 1: key exists directly (normal key or device-id-as-key)
    if k in keys:
        kd = keys[k]
        if not kd.get("active", False):
            return jsonify({"valid": False, "msg": "Key inactive"})
        ...
        if kd.get("expiry") and kd["expiry"] < today:
            return jsonify({"valid": False, "msg": "Key expired"})
        stored_dev = (kd.get("device") or "").strip().upper()
        # Device ID = Key scenario: stored_dev == k, allow if device matches or is same as key
    ...
    # Case 2: user typed their device ID — find key where device == k
...
def api_ping():
    return jsonify({"status": "ok", "msg": "pong", "app": "POSCUS PREDICTER"})
