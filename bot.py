...
# ─── CONFIG ──────────────────────────────────────────────────
BOT_TOKEN   = os.environ.get("BOT_TOKEN", "8726110607:AAG0RwxrfNa0btUCQ5A-YA-aMtlE03ale8s")
...
def expiry_inline_kb(prefix="exp"):
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("1 Hour",   callback_data=f"{prefix}_h1"),
         ...
         InlineKeyboardButton("4 Hours",  callback_data=f"{prefix}_h4"),
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid):
        await update.message.reply_text(
            f"👑 *Welcome to Poscus Admin*\n\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"🤖 *POSCUS ADMIN BOT* v2.0\n"
            f"━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Hello `{update.effective_user.first_name}`! 👋\n\n"
            f"🌐 Web: `{RENDER_URL}`\n"
            f"🔑 Admin Panel: `{RENDER_URL}/admin`\n\n"
...
async def help_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        ...
        return
    await update.message.reply_text(
        "📖 *POSCUS BOT — ALL COMMANDS*\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🔧 *GENERAL*\n"
        "/start — Start bot & show menu\n"
        "/help — Show this help message\n"
        "/allurl — Show all web URLs\n"
        "/ping — Check web server status\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "🔑 *KEY MANAGEMENT* _(via keyboard)_\n"
        "➕ Add Key — Add new key\n"
        "📋 All Keys — List all keys\n"
        "🔍 Search Key — Search key/user\n"
        "🗑️ Delete Key — Delete a key\n"
        "📱 Reset Device — Unlock device\n"
        "⚡ Bulk Generate — Generate many keys\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        "📊 *STATS & MANAGEMENT*\n"
        "📊 Stats — View all stats\n"
        "👥 Resellers — Manage resellers\n"
        "🗑️ Del Expired — Delete expired keys\n"
        "❌ Deact All — Deactivate all keys\n\n"
        "━━━━━━━━━━━━━━━━━━━━\n"
        f"🌐 `{RENDER_URL}`",
        parse_mode='Markdown',
        reply_markup=admin_main_kb()
...
# ─── /allurl ─────────────────────────────────────────────────
async def allurl_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    if not is_admin(uid):
        await update.message.reply_text("⛔ Access denied!")
        return
    await update.message.reply_text(
        ...
        f"🔑 *Admin Panel:*\n`{RENDER_URL}/admin`\n\n"
        f"📡 *API Endpoints:*\n"
        f"`{RENDER_URL}/api/key/validate`\n"
        f"`{RENDER_URL}/api/key/verify`\n"
        f"`{RENDER_URL}/api/admin/keys`\n"
        f"`{RENDER_URL}/api/admin/resellers`\n\n"
        "━━━━━━━━━━━━━━━━━━━━",
        parse_mode='Markdown',
        reply_markup=admin_main_kb()
    )
...
async def ping_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(uid):
        ...
        return
    import time
    msg = await update.message.reply_text("⏳ Pinging server...")
    try:
        start = time.time()
    ...
    await msg.edit_text(
        f"📡 *SERVER PING RESULT*\n\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"{icon} Status: *{stat_text}*\n"
        f"⚡ Response: `{elapsed}ms`\n"
        f"🌐 URL: `{RENDER_URL}`\n"
        f"━━━━━━━━━━━━━━━━━━━━",
        parse_mode='Markdown'
    )
...
def main():
    ...
    app.add_handler(CommandHandler("allurl", allurl_cmd))
